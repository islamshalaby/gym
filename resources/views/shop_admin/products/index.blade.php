            ...
            <div class="widget-content widget-content-area">
                ...
                @if($data['expire'] == 'no')
                    <a class="btn btn-primary"
                       href="/admin-panel/products/show?expire=soon">{{ __('messages.expired_soon') }}</a>
                @endif

                @if($data['expire'] == 'soon')
                    <a class="btn btn-primary"
                       href="/admin-panel/products/show">{{ __('messages.return_all_products') }}</a>
                @endif
            </div>
                        ...
                        <div class="row">
                            <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                <h4>{{ __('messages.show_products') }}
                                    <button data-show="0"
                                            class="btn btn-primary show_actions">{{ __('messages.hide_actions') }}</button>
                                </h4>
                        ...
                        <div class="table-responsive">
                            <table id="html5-extension" class="table table-hover non-hover">
                                ...
                                @foreach ($data['products'] as $product)
                                    <tr>
                                        <td><?=$i;?></td>
                                        <td class="hide_col"><img style="width: 50px"
                                                                  src="{{image_cloudinary_url()}}{{ isset($product->mainImage->image) ? $product->mainImage->image : '' }}"/>
                                        </td>
                                        ...
                                        <td>
                                            @if($product->free == 0)
                                                <a href="{{ route('products.action.offer', [$product->id, 1]) }}" class="btn btn-success"> {{ __('messages.add_to_offer') }}</a>
                                            @else
                                                <a href="{{ route('products.action.offer', [$product->id, 0]) }}" class="btn btn-danger">{{ __('messages.remove_from_offer') }}</a>
                                            @endif
                                        </td>
                                        <td>
                                            @if($product->multi_options == 0)
                                                            ...
                                                            @foreach ($product->multiOptions as $m_option)
                                                                @if (!empty($m_option->barcode ))
                                                                    <a class="dropdown-item"
                                                                       href="javascript:void(0);">{{ $m_option->barcode }}</a>
                                                                @endif
                                                            @endforeach
                                                        </div>
                                                    </div>
                                        ...
                                        <td class="hide_col">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuReference5"
                                                     style="will-change: transform;">
                                                    <a class="dropdown-item"
                                                       href="{{ route('products.details', $product->id) }}">{{ __('messages.details') }}</a>
                                                    <a class="dropdown-item"
                                                       href="{{ route('products.edit', $product->id) }}">{{ __('messages.edit') }}</a>
                                                    @if($product->reviewed == 0)
                                                        <a class="dropdown-item"
                                                           onclick='return confirm("{{ __('messages.are_you_sure') }}");'
                                                           href="{{ route('products.review', [$product->id, 1]) }}">{{ __('messages.publish') }}</a>
                                                    @endif
                                                    <a class="dropdown-item"
                                                       onclick='return confirm("{{ __('messages.are_you_sure') }}");'
                                                       href="{{ route('products.delete', $product->id) }}">{{ __('messages.delete') }}</a>
                                                    <div class="dropdown-divider"></div>
                                                    @if($product->hidden == 0)
                                                        <a class="dropdown-item"
                                                           onclick='return confirm("{{ __('messages.are_you_sure') }}");'
                                                           href="{{ route('products.visibility.status', [$product->id, 1]) }}">{{ __('messages.hide_product') }}</a>
                                                    @else
                                                        <a class="dropdown-item"
                                                           onclick='return confirm("{{ __('messages.are_you_sure') }}");'
                                                           href="{{ route('products.visibility.status', [$product->id, 0]) }}">{{ __('messages.show_product') }}</a>
                                                    @endif
                                                </div>
                                            </div>
                                        </td>
